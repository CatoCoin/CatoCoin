...
Value getinfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total catocoin balance of the wallet (excluding zerocoins)\n"
            ...
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in catocoin/kb\n"
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zcatocoinObj.push_back(Pair(to_string(denom), ValueFromAmount(chainActive.Tip()->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zcatocoinObj.emplace_back(Pair("total", ValueFromAmount(chainActive.Tip()->GetZerocoinSupply())));
    obj.emplace_back(Pair("zCATOsupply", zcatocoinObj));
...
if (chainActive.Height() >= 495217) {
        obj.push_back(Pair("MN reward", GetMasternodePayment(chainActive.Height(), GetBlockValue(chainActive.Height()), 333) * 0.00000001));
        obj.push_back(Pair("Staking reward", (0.00000001*GetBlockValue(chainActive.Height())) - (0.00000001 *GetMasternodePayment(chainActive.Height(), GetBlockValue(chainActive.Height()), 333))));
int count_tier_1 = 0;
int count_tier_2 = 0;
int count_tier_3 = 0;
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nReturn information about the given catocoin address.\n"
            "\nArguments:\n"
            "1. \"catocoinaddress\"     (string, required) The catocoin address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"catocoinaddress\", (string) The catocoin address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: Catocoin address and we have full public key:
...
Value createmultisig(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2) {
        string msg = "createmultisig nrequired [\"key\",...]\n"
                     ...
                     "2. \"keys\"       (string, required) A json array of keys which are catocoin addresses or hex-encoded public keys\n"
                     "     [\n"
                     "       \"key\"    (string) catocoin address or hex-encoded public key\n"
                     "       ,...\n"
                     "     ]\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"catocoinaddress\"  (string, required) The catocoin address to use for the signature.\n"
