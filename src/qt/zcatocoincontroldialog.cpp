...
#include "ui_zcatocoincontroldialog.h"
...
std::list<std::string> ZCatocoinControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZCatocoinControlDialog::listMints;

ZCatocoinControlDialog::ZCatocoinControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZCatocoinControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZCatocoinControlDialog::~ZCatocoinControlDialog()
{
    delete ui;
}

void ZCatocoinControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZCatocoinControlDialog::updateList()
{
...
void ZCatocoinControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZCatocoinControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZCatocoin_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZCatocoinControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZCatocoinControlDialog::GetSelectedMints()
{
...
void ZCatocoinControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
